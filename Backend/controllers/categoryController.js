...
module.exports.getCategories = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "categories data", success: true, data: categories });
  } catch (error) {
    console.log(error);
...
module.exports.getCategoryById = async (req, res) => {
  try {
    ...
    if (!parent_id) {
      return res
        .status(200)
        .json({ success: false, message: "category id not found", data: [] });
    }
  ...
  try {
    ...
    if (req.file) {
      await Category.updateOne(
        { _id: req.body._id },
        { $set: { name: req.body.name, category_image: req.file.originalname } }
      );
    } else {
